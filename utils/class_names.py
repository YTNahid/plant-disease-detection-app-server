...
def get_class_name(index: int) -> str:
    """Return the exact class label for a given output index."""
...
def get_all_classes() -> Dict[int, str]:
    """Return a dict of {index: label} for all classes."""
    return {i: name for i, name in enumerate(CLASS_NAMES)}


def get_disease_info(index_or_name) -> DiseaseInfo:
    """Return the disease metadata for a given class index or class name."""
